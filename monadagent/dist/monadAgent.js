...
// import axios from "axios"; // Simplified import, no AxiosResponse needed
...
//     return `Balance: ${ethers.formatEther(balance)} MONAD`;
//   }
// }
...
//       return `Transferred ${amount} MONAD to ${to}. Tx: ${txResponse.hash}`;
//     } catch (error) {
...
//       data: log.data,
//     }));
//     return `Recent ${count} transactions: ${JSON.stringify(recentTxs, null, 2)}`;
//   }
// }
...
//   async _call() {
//     try {
//       const response = await axios.get<{ pairs: { baseToken: { symbol: string }; priceUsd: string }[] }>(
//         "https://api.dexscreener.com/latest/dex/tokens"
//       );
//       const trending = response.data.pairs.slice(0, 5).map((pair: { baseToken: { symbol: string }; priceUsd: string }) => ({
//         token: pair.baseToken.symbol,
//         price: pair.priceUsd,
//       }));
//       return `Trending tokens: ${JSON.stringify(trending, null, 2)}`;
//     } catch (error) {
//       log.error("GetTrendingTokensTool error:", error);
//       throw new Error(`Failed to fetch trending tokens: ${error instanceof Error ? error.message : String(error)}`);
//     }
//   }
// }
...
// const MONAD_TOKENS_URL = `${MONAD_EXPLORER_URL}/tokens`;
// // Logger setup
// const log = new Logger({ name: "MonadAgent" });
// // Initialize OpenAI model
// const llm = new ChatOpenAI({
...
//     const provider = this.tools.getProvider();
//     const blockNumber = await provider.getBlockNumber();
//     const fromBlock = Math.max(blockNumber - 99, 0); // Limit to 100 blocks per Monad RPC restriction
//     const filter = { fromBlock, toBlock: blockNumber, address: wallet.address };
//     try {
//       const logs = await provider.getLogs(filter);
//       const recentTxs = logs.slice(0, count).map((log) => ({
...
//   async _call() {
//     try {
//       const response = await axios.get<string>(MONAD_TOKENS_URL, {
//         headers: { "User-Agent": "Mozilla/5.0" }, // Add user-agent to avoid potential blocks
//       });
//       const $ = cheerio.load(response.data); // Type response.data as string
//       const tokens: { token: string; price: string }[] = [];
//       // Adjust selectors based on actual HTML structure of https://monad-testnet.socialscan.io/tokens
//       $("table tbody tr").each((_, element) => {
//         const tokenName = $(element).find("td:nth-child(1)").text().trim(); // Token name column
//         const price = $(element).find("td:nth-child(2)").text().trim(); // Price column (adjust index)
//         if (tokenName && price) {
...
// import bodyParser from "body-parser";
// import axios from "axios";
// import * as cheerio from "cheerio"; // Updated import to ensure full module
// import { Logger } from "tslog";
// dotenv.config();
// // Environment variables
// const OPENAI_API_KEY = process.env.OPENAI_API_KEY || "";
...
//     try {
//       if (!cheerio) {
//         throw new Error("Cheerio module is not available. Please ensure it is installed.");
//       }
//       const response = await axios.get<string>(MONAD_TOKENS_URL, {
//         headers: { "User-Agent": "Mozilla/5.0" },
//       });
//       const $ = cheerio.load(response.data); // cheerio.load should now work
//       const tokens: { token: string; price: string }[] = [];
//       // Adjust selectors based on actual HTML structure of https://monad-testnet.socialscan.io/tokens
//       $("table tbody tr").each((_, element) => {
//         const tokenName = $(element).find("td:nth-child(1)").text().trim(); // Token name column
