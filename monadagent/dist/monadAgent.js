...
class GetBalanceTool extends tools_1.StructuredTool {
    ...
    async _call() {
        ...
        for (const [tokenName, tokenAddress] of Object.entries(tokenMap)) {
            try {
                const tokenContract = new ethers_1.ethers.Contract(tokenAddress, ERC20_ABI, this.tools.getProvider());
                const balance = await tokenContract.balanceOf(wallet.address);
                balances.push(`${tokenName} Balance: ${ethers_1.ethers.formatUnits(balance, 18)} ${tokenName}`);
            }
            catch (error) {
...
class GetTokenPriceTool extends tools_1.StructuredTool {
    ...
    async _call({ token }) {
        try {
            const response = await axios_1.default.get(`https://api.coingecko.com/api/v3/simple/price?ids=${token.toLowerCase()}&vs_currencies=usd`, { headers: { "x-cg-api-key": COINGECKO_API_KEY } });
...
class GetTrendingTokensTool extends tools_1.StructuredTool {
    ...
    async _call() {
        try {
            ...
            const response = await axios_1.default.get(`${MONAD_EXPLORER_URL}/tokens`, {
...
class CreateTokenTool extends tools_1.StructuredTool {
    ...
    async _call({ name, symbol, totalSupply }) {
        const wallet = this.tools.getWallet();
        if (!wallet)
...
class BatchMixedTransferTool extends tools_1.StructuredTool {
    tools;
    schema = zod_1.z.object({
        transfers: zod_1.z
            .string()
            .describe("A space-separated list of mixed transfers in the format '<type1> <to1> <amount1> [tokenName1] <type2> <to2> <amount2> [tokenName2]'. " +
    ...
    description = "Transfer MONAD and ERC-20 tokens in a single batch using token names. Format: batchMixedTransfer <type1> <to1> <amount1> [tokenName1] <type2> <to2> <amount2> [tokenName2] ...";
    constructor(tools) {
        super();
        this.tools = tools;
    }
    async _call({ transfers }) {
        const wallet = this.tools.getWallet();
        if (!wallet)
            return "No wallet set. Please set a wallet with 'setWallet <privateKey>' first.";
        ...
        for (const [index, { type, to, amount, tokenName }] of transferList.entries()) {
            try {
                if (type === "MONAD") {
                    ...
                    const tokenContract = new ethers_1.ethers.Contract(tokenAddress, ERC20_ABI, wallet);
                    ...
                    if (receipt && receipt.hash) {
                        ...
                        results.push(`${index + 1}. **${tokenName} Transfer to ${to}**:\n   - Amount: ${amount} ${tokenName}\n   - Status: Successful\n   - Transaction Link: [View Transaction](${MONAD_EXPLORER_URL}/tx/${receipt.hash})`);
                    }
                    else {
                        throw new Error("Transaction receipt is null or invalid");
                    }
                ...
                results.push(`${index + 1}. **${type === "MONAD" ? "MONAD" : tokenName} Transfer to ${to}**:\n   - Amount: ${amount} ${type === "MONAD" ? "MONAD" : tokenName}\n   - Status: Failed\n   - Error: ${errorMsg}`);
            }
        }
...
async function callAgent(state) {
    const systemMessage = new messages_1.SystemMessage("You are an AI assistant that helps users interact with the Monad Testnet blockchain. Use the provided tools to assist the user. The wallet private key persists until the user explicitly disconnects.");
